...
void usage(char *prog_name, char *filename) {
   printf("Usage: %s <test submission %s>\n", prog_name , filename);
   exit(0);
}
...
int main(int argc, char *argv[]) {
   ...
   char *chall_name, *buffer, *datafile;

   buffer = (char *) ec_malloc(MAXBUFFERSIZE);
   datafile = (char *) ec_malloc(MAXFILESIZE);
   chall_name = (char *) ec_malloc(MAXFILENAMELENGTH);
   printf("Enter chall_name: \n");
   scanf("%s", chall_name);
   snprintf(datafile, 250, "/home/betsythecat/challenges_test/%s", chall_name);
